...
public class ReadExcel
{
	 ...
	 * @throws Exception
	 */
	public static String[] impotrHead(String path,
			Map<String, String> tableTitle) throws Exception
	{
		...
		{
			if (tableTitle.containsKey(ss[i]))
			{
				// 如果包含的话，则更换
				if (headName.equals(""))
				{
					headName = tableTitle.get(ss[i]);
				} else
				{
					headName = headName + "," + tableTitle.get(ss[i]);
				}
			} else
			{
				// 不包含则报错，退出
			...
			Map<Integer, String> excelMaps) throws Exception
	{
		...
		for (int i = 1; i <= excelMaps.size(); i++)
		{
			...
			for (int j = 0; j < fieldNames.length; j++)
			{
				...
				/*
				 * target 指定对象 fieldName 属性名 argTypes 参数类型 args 参数列表
				 */
